...
function New-OrphanedSIDResult {
    ...
    try {
        Write-StructuredLog "Creating OrphanedSIDResult object for $ObjectDN" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        ...
        Write-StructuredLog "Successfully created OrphanedSIDResult object for $ObjectDN with SID $OrphanedSID" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId

        return $result
    }
    catch {
        Write-StructuredLog "Error creating OrphanedSIDResult object for $ObjectDN : $($_.Exception.Message)" -Level Error -Component 'ResultFactory' -CorrelationId $CorrelationId
...
function Add-ProcessingMetadata {
    ...
    try {
        Write-StructuredLog "Adding processing metadata to result object" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        ...
        Write-StructuredLog "Successfully added processing metadata with correlation ID $CorrelationId" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
    }
    catch {
        Write-StructuredLog "Error adding processing metadata: $($_.Exception.Message)" -Level Warning -Component 'ResultFactory' -CorrelationId $CorrelationId
        throw
    }
}
...
function Set-ACLMetadata {
    ...
    try {
        Write-StructuredLog "Extracting ACL metadata from access rule" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        ...
        if ($AccessRule.PSObject.Properties['AccessControlType']) {
            ...
            Write-StructuredLog "Set AccessControlType: $($Result.AccessControlType)" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        } else {
            $Result.AccessControlType = "Unknown"
            Write-StructuredLog "AccessControlType property missing, set to Unknown" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        }
        ...
        if ($AccessRule.PSObject.Properties['ActiveDirectoryRights']) {
            ...
            Write-StructuredLog "Set ActiveDirectoryRights: $($Result.ActiveDirectoryRights)" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        } else {
            # Try alternative property names
            if ($AccessRule.PSObject.Properties['Rights']) {
                $Result.ActiveDirectoryRights = $AccessRule.Rights
                Write-StructuredLog "Set ActiveDirectoryRights from Rights property: $($Result.ActiveDirectoryRights)" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
            } else {
                $Result.ActiveDirectoryRights = "Unknown"
                Write-StructuredLog "ActiveDirectoryRights property missing, set to Unknown" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
            }
        }
        ...
        if ($AccessRule.PSObject.Properties['InheritanceFlags']) {
            ...
            Write-StructuredLog "Set InheritanceType: $($Result.InheritanceType)" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        } else {
            $Result.InheritanceType = "None"
            Write-StructuredLog "InheritanceFlags property missing, set to None" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        }
        ...
        if ($AccessRule.PSObject.Properties['IsInherited']) {
            ...
            Write-StructuredLog "Set IsInherited: $($Result.IsInherited)" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        } else {
            $Result.IsInherited = $false
            Write-StructuredLog "IsInherited property missing, set to false" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
        }

        Write-StructuredLog "Successfully extracted all available ACL metadata" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
    }
    catch {
        Write-StructuredLog "Error extracting ACL metadata: $($_.Exception.Message)" -Level Warning -Component 'ResultFactory' -CorrelationId $CorrelationId
        throw
    }
}
        ...
        if ($Object.PSObject.Properties[$PropertyName] -and $Object.$PropertyName) {
            ...
            if ($guidValue -is [System.Guid]) {
                if ($guidValue -eq [System.Guid]::Empty) {
                    Write-StructuredLog "GUID property $PropertyName is empty GUID" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
                    return [System.Guid]::Empty
                } else {
                    Write-StructuredLog "Set $PropertyName to GUID: $guidValue" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
                    return $guidValue
                }
            } elseif ($guidValue -is [string]) {
                if ([string]::IsNullOrWhiteSpace($guidValue) -or $guidValue -eq "00000000-0000-0000-0000-000000000000") {
                    Write-StructuredLog "GUID property $PropertyName is empty string or zero GUID" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
                    return [System.Guid]::Empty
                } else {
                    try {
                        $parsedGuid = [System.Guid]::Parse($guidValue)
                        Write-StructuredLog "Set $PropertyName to parsed GUID: $parsedGuid" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
                        return $parsedGuid
                    } catch {
                        Write-StructuredLog "Failed to parse GUID string for $PropertyName : $guidValue" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
                        return [System.Guid]::Empty
                    }
                }
            } else {
                Write-StructuredLog "GUID property $PropertyName has unexpected type: $($guidValue.GetType().Name)" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
                return [System.Guid]::Empty
            }
        } else {
            Write-StructuredLog "GUID property $PropertyName missing or null, set to Empty" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
            return [System.Guid]::Empty
        }
    }
    catch {
        Write-StructuredLog "Error extracting GUID property $PropertyName : $($_.Exception.Message)" -Level Warning -Component 'ResultFactory' -CorrelationId $CorrelationId
        return [System.Guid]::Empty
    }
}
...
function ConvertTo-ResultSummary {
    ...
    process {
        try {
            Write-StructuredLog "Creating result summary for $($OrphanedSIDResults.Count) orphaned SID results" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId
            ...
            Write-StructuredLog "Successfully created result summary with $($summary.TotalOrphanedSIDs) total orphaned SIDs" -Level Debug -Component 'ResultFactory' -CorrelationId $CorrelationId

            return $summary
        }
        catch {
            Write-StructuredLog "Error creating result summary: $($_.Exception.Message)" -Level Error -Component 'ResultFactory' -CorrelationId $CorrelationId
            throw
        }
    }
}
...
Write-StructuredLog "New-SIDResult module loaded successfully" -Level Debug -Component 'ResultFactory' -CorrelationId $([System.Guid]::NewGuid().ToString())
