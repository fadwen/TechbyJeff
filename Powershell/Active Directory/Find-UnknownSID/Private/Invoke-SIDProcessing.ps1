...
function Find-OrphanedSIDsInObject {
    ...
    process {
        foreach ($obj in $ADObject) {
            ...
            try {
                Write-StructuredLog "Starting orchestrated orphaned SID detection for object" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId

                # Handle error objects that may come through the pipeline
                if ($obj.PSObject.Properties['IsError'] -and $obj.IsError -eq $true) {
                    Write-StructuredLog "Skipping error object: $($obj.ErrorMessage)" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                ...
                if (-not $obj.DistinguishedName) {
                    Write-StructuredLog "Object missing DistinguishedName property, skipping" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                ...
                Write-StructuredLog "Processing object: $objectDN" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId

                # Check for security descriptor
                if (-not $obj.nTSecurityDescriptor) {
                    Write-StructuredLog "No security descriptor found for $objectDN" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                    $processingStats.SkippedSIDs++
                ...
                if (-not $accessRules) {
                    Write-StructuredLog "No access rules retrieved for $objectDN" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                    $processingStats.SkippedSIDs++
                    return $results.ToArray()
                }

                $processingStats.AccessRulesRetrieved = $accessRules.Count
                Write-StructuredLog "Retrieved $($accessRules.Count) access rules for processing" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                ...
                foreach ($rule in $accessRules) {
                    try {
                        ...
                        if ($orphanedResult) {
                            ...
                            Write-StructuredLog "Orphaned SID detected and result object created for $objectDN" -Level Verbose -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                        }

                        $processingStats.ProcessedSIDs++
                    }
                    catch {
                        Write-StructuredLog "Error processing access rule for $objectDN : $($_.Exception.Message)" -Level Warning -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                        $processingStats.SkippedSIDs++
                        $processingStats.ErrorsEncountered++
                    }
                }
                ...
                Write-StructuredLog "Completed processing $objectDN - Processed: $($processingStats.ProcessedSIDs), Orphaned found: $($processingStats.OrphanedSIDsFound), Duration: $($processingStats.Duration)ms" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                ...
                if ($processingStats.ProcessedSIDs -gt 0 -and $processingStats.ProcessedSIDs % 100 -eq 0) {
                    Write-StructuredLog "Performing periodic garbage collection after processing $($processingStats.ProcessedSIDs) SIDs" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                    [System.GC]::Collect()
                ...
                $processingStats.ErrorsEncountered++
                $processingStats.EndTime = Get-Date
                Write-StructuredLog "Error in orchestrated processing for object $($processingStats.ObjectDN) : $($_.Exception.Message)" -Level Error -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId

                # Log final processing statistics even on error
                Write-StructuredLog "Processing statistics for $($processingStats.ObjectDN) : Processed: $($processingStats.ProcessedSIDs), Errors: $($processingStats.ErrorsEncountered), Orphaned found: $($processingStats.OrphanedSIDsFound)" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
...
function Get-SIDProcessingStatistics {
    ...
    process {
        try {
            Write-StructuredLog "Generating processing statistics for $($OrphanedSIDResults.Count) results" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
            ...
            if (-not $allStats) {
                Write-StructuredLog "No processing statistics found in results" -Level Warning -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
                return $null
            }
            ...
            Write-StructuredLog "Generated comprehensive processing statistics: $($statistics.TotalObjectsProcessed) objects, $($statistics.TotalSIDsProcessed) SIDs, $($statistics.TotalOrphanedSIDsFound) orphaned" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId

            return $statistics
        }
        catch {
            Write-StructuredLog "Error generating processing statistics: $($_.Exception.Message)" -Level Error -Component 'SIDProcessingOrchestration' -CorrelationId $CorrelationId
            throw
        }
    }
}
...
Write-StructuredLog "Invoke-SIDProcessing module loaded successfully" -Level Debug -Component 'SIDProcessingOrchestration' -CorrelationId $([System.Guid]::NewGuid().ToString())
