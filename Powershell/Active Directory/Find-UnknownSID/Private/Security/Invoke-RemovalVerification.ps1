...
function Invoke-RemovalVerification {
    ...
    try {
        Write-StructuredLog "Starting removal verification for $ObjectDN (Verifying $($AllowedSIDs.Count) SIDs)" -Level Debug -CorrelationId $CorrelationId
        ...
        if ($remainingOrphanedSIDs.Count -eq 0) {
            Write-StructuredLog "Verification successful: All $($AllowedSIDs.Count) SIDs removed from $ObjectDN" -Level Verbose -CorrelationId $CorrelationId
            ...
            Write-StructuredLog "Verification failed for $ObjectDN : $errorMessage" -Level Warning -CorrelationId $CorrelationId
            return [PSCustomObject]@{
                Success = $false
        ...
        Write-StructuredLog "Verification error for $ObjectDN : $errorMessage" -Level Error -CorrelationId $CorrelationId
        return [PSCustomObject]@{
            Success = $false
            ErrorMessage = $errorMessage
            RemainingOrphanedSIDs = @()
...
Write-StructuredLog "Removal verification module loaded successfully" -Level Debug -CorrelationId $([System.Guid]::NewGuid().ToString())
