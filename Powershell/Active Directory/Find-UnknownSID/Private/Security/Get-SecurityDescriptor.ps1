...
function Get-ObjectAccessRule {
    ...
    process {
        try {
            ...
            Write-StructuredLog "Starting security descriptor retrieval for: $objectDN (IncludeInherited: $IncludeInherited)" -Level Debug -CorrelationId $CorrelationId
            ...
            if ($accessRules) {
                Write-StructuredLog "Successfully retrieved access rules using Get-Acl method for $objectDN" -Level Debug -CorrelationId $CorrelationId
                return $accessRules
            }
            ...
            if ($accessRules) {
                Write-StructuredLog "Successfully retrieved access rules using binary processing for $objectDN" -Level Debug -CorrelationId $CorrelationId
                return $accessRules
            }
            ...
            if ($accessRules) {
                Write-StructuredLog "Successfully retrieved access rules using fresh AD query for $objectDN" -Level Debug -CorrelationId $CorrelationId
                return $accessRules
            }

            # All methods failed
            Write-StructuredLog "All security descriptor retrieval methods failed for $objectDN" -Level Warning -CorrelationId $CorrelationId
            return $null
        }
        catch {
            Write-StructuredLog "Unexpected error retrieving access rules for $($ADObject.DistinguishedName): $($_.Exception.Message)" -Level Error -CorrelationId $CorrelationId
            return $null
        }
    }
}
...
function Get-SecurityDescriptorFromGetAcl {
    ...
    try {
        $objectDN = $ADObject.DistinguishedName
        Write-StructuredLog "Attempting Get-Acl retrieval for: $objectDN" -Level Debug -CorrelationId $CorrelationId

        $acl = Get-Acl -Path "AD:\$objectDN" -ErrorAction Stop

        if (-not $acl -or -not $acl.Access) {
            Write-StructuredLog "Get-Acl returned null or empty access list for $objectDN" -Level Debug -CorrelationId $CorrelationId
            return $null
        }
        ...
        Write-StructuredLog "Get-Acl successfully retrieved $($accessRules.Count) access rules for $objectDN" -Level Debug -CorrelationId $CorrelationId
        return $accessRules
    }
    catch {
        Write-StructuredLog "Get-Acl failed for $($ADObject.DistinguishedName): $($_.Exception.Message)" -Level Debug -CorrelationId $CorrelationId
        return $null
    }
}
...
function Get-SecurityDescriptorFromBinary {
    ...
    try {
        ...
        if (-not $binarySD) {
            Write-StructuredLog "No nTSecurityDescriptor property found for $objectDN" -Level Debug -CorrelationId $CorrelationId
            return $null
        }

        Write-StructuredLog "Processing binary security descriptor for: $objectDN" -Level Debug -CorrelationId $CorrelationId
        ...
        if ($binarySD -is [System.DirectoryServices.ActiveDirectorySecurity]) {
            Write-StructuredLog "Processing regular ActiveDirectorySecurity object for $objectDN" -Level Debug -CorrelationId $CorrelationId
            ...
            Write-StructuredLog "Successfully processed ActiveDirectorySecurity object for $objectDN - Found $($accessRules.Count) rules" -Level Debug -CorrelationId $CorrelationId
            return $accessRules
        }
        ...
        Write-StructuredLog "Binary security descriptor processing failed for $($ADObject.DistinguishedName): $($_.Exception.Message)" -Level Debug -CorrelationId $CorrelationId
        return $null
    }
}
...
function Get-SecurityDescriptorFromDeserialized {
    ...
    try {
        Write-StructuredLog "Processing deserialized ActiveDirectorySecurity object for $ObjectDN" -Level Debug -CorrelationId $CorrelationId

        if (-not $SecurityDescriptor.Access) {
            Write-StructuredLog "Deserialized security descriptor has no Access property for $ObjectDN" -Level Debug -CorrelationId $CorrelationId
            return $null
        }
        ...
        Write-StructuredLog "Successfully extracted $($accessRules.Count) access rules from deserialized security descriptor for $ObjectDN" -Level Debug -CorrelationId $CorrelationId
        return $accessRules
    }
    catch {
        Write-StructuredLog "Failed to process deserialized security descriptor for $ObjectDN : $($_.Exception.Message)" -Level Warning -CorrelationId $CorrelationId
        return $null
    }
}
...
function ConvertTo-SecurityDescriptor {
    ...
    try {
        Write-StructuredLog "Converting binary data to security descriptor for $ObjectDN" -Level Debug -CorrelationId $CorrelationId
        ...
        if ($BinaryData -is [byte[]]) {
            Write-StructuredLog "Processing byte array security descriptor" -Level Debug -CorrelationId $CorrelationId
            $securityDescriptor.SetSecurityDescriptorBinaryForm($BinaryData)
        }
        elseif ($BinaryData -is [string]) {
            Write-StructuredLog "Processing Base64 string security descriptor" -Level Debug -CorrelationId $CorrelationId
            $byteArray = [System.Convert]::FromBase64String($BinaryData)
            $securityDescriptor.SetSecurityDescriptorBinaryForm($byteArray)
        }
        else {
            Write-StructuredLog "Converting generic binary data to byte array" -Level Debug -CorrelationId $CorrelationId
        ...
        Write-StructuredLog "Successfully converted binary security descriptor for $ObjectDN - Found $($accessRules.Count) rules" -Level Debug -CorrelationId $CorrelationId
        ...
        Write-StructuredLog "Failed to convert binary security descriptor for $ObjectDN : $($_.Exception.Message)" -Level Warning -CorrelationId $CorrelationId
        return $null
    }
}
...
function Invoke-FreshADQuery {
    ...
    try {
        $objectDN = $ADObject.DistinguishedName
        Write-StructuredLog "Attempting fresh AD query for security descriptor: $objectDN" -Level Debug -CorrelationId $CorrelationId
        ...
        if (-not $freshObj.nTSecurityDescriptor) {
            Write-StructuredLog "Fresh AD query returned no security descriptor for $objectDN" -Level Warning -CorrelationId $CorrelationId
            return $null
        }
        ...
        Write-StructuredLog "Fresh AD query successfully retrieved $($accessRules.Count) access rules for $objectDN" -Level Debug -CorrelationId $CorrelationId
        ...
        Write-StructuredLog "Fresh AD query failed for $($ADObject.DistinguishedName): $($_.Exception.Message)" -Level Warning -CorrelationId $CorrelationId
        return $null
    }
}
...
Write-StructuredLog "Get-SecurityDescriptor module loaded successfully" -Level Debug -CorrelationId $([System.Guid]::NewGuid().ToString())
