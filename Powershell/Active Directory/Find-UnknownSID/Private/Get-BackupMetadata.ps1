...
function Get-BackupMetadata {
    ...
    begin {
        Write-StructuredLog "Starting backup metadata extraction operation" -Level Debug -Component 'BackupMetadata' -CorrelationId $CorrelationId
    }
    ...
    process {
        try {
            ...
            Write-StructuredLog "Extracting metadata from backup: $sanitizedPath" -Level Debug -Component 'BackupMetadata' -CorrelationId $CorrelationId

            # Validate file existence and accessibility
            if (-not (Test-Path $sanitizedPath -PathType Leaf)) {
                $errorMessage = "Backup file not found or inaccessible: $sanitizedPath"
                Write-StructuredLog $errorMessage -Level Error -Component 'BackupMetadata' -CorrelationId $CorrelationId
            ...
            Write-StructuredLog "Successfully extracted metadata for backup: $($backupData.ObjectDN)" -Level Verbose -Component 'BackupMetadata' -CorrelationId $CorrelationId
            ...
            Write-StructuredLog "Failed to extract metadata from backup file" -Level Error -Component 'BackupMetadata' -Details $errorDetails -CorrelationId $CorrelationId
        ...
        Write-StructuredLog "Backup metadata extraction operation completed" -Level Debug -Component 'BackupMetadata' -CorrelationId $CorrelationId
    }
}

Write-StructuredLog "Get-BackupMetadata module loaded successfully" -Level Debug -Component 'BackupMetadata' -CorrelationId $([System.Guid]::NewGuid().ToString())
