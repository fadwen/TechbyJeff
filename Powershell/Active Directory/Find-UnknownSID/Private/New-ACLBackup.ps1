    ...
    process {
        try {
            ...
            if (-not $ACL.Access -or $ACL.Access.Count -eq 0) {
                Write-StructuredLog "ACL object contains no access entries to backup for $($ObjectDN.Trim())" -Level Warning -Component 'BackupCreation' -CorrelationId $CorrelationId
            ...
            if ($safeName.Length -gt 200) {
                ...
                Write-StructuredLog "Filename truncated for length: $safeName" -Level Debug -Component 'BackupCreation' -CorrelationId $CorrelationId
            }
            ...
            if (-not (Test-Path $BackupPath)) {
                ...
                Write-StructuredLog "Created backup directory: $BackupPath" -Level Information -Component 'BackupCreation' -CorrelationId $CorrelationId
            }
            ...
            if ($PSCmdlet.ShouldProcess($ObjectDN, "Create ACL Backup")) {
                ...
                Write-StructuredLog "Backup creation was cancelled by user" -Level Warning -Component 'ACLBackup' -CorrelationId $CorrelationId
                return $false
            }

            # Success logging with comprehensive details
            $fileSize = (Get-Item $backupFile).Length
            Write-StructuredLog "Successfully backed up ACL to: $backupFile (Size: $fileSize bytes, Entries: $($ACL.Access.Count), Hash: $($verifyData.SDDLHash.Substring(0,8))...)" -Level Information -Component 'BackupCreation' -CorrelationId $CorrelationId
            ...
            Write-StructuredLog "Failed to create ACL backup for $($ObjectDN.Trim()) : $($_.Exception.Message)" -Level Error -Component 'BackupCreation' -CorrelationId $CorrelationId -Data $errorDetails

            # Cleanup any partial backup files on failure
            if ($backupFile -and (Test-Path $backupFile -ErrorAction SilentlyContinue)) {
                Remove-Item $backupFile -Force -ErrorAction SilentlyContinue
                Write-StructuredLog "Cleaned up partial backup file: $backupFile" -Level Debug -Component 'BackupCreation' -CorrelationId $CorrelationId
            }
...
Write-StructuredLog "New-ACLBackup module loaded successfully" -Level Debug -Component 'BackupCreation' -CorrelationId $([System.Guid]::NewGuid().ToString())
