...
function Test-AccessRuleForOrphanedSID {
    ...
    try {
        Write-StructuredLog "Starting access rule analysis for orphaned SID (Object: $ObjectDN)" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
        ...
        if (-not $AccessRule -or -not $AccessRule.PSObject.Properties['IdentityReference'] -or -not $AccessRule.IdentityReference) {
            Write-StructuredLog "Skipping access rule with missing or null IdentityReference for $ObjectDN" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $null
        }
        ...
        if (-not $sidString) {
            Write-StructuredLog "Failed to resolve identity reference to SID for $ObjectDN" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $null
        }
        ...
        if (-not (Test-SIDValidityAndType -SIDString $sidString -CorrelationId $CorrelationId)) {
            Write-StructuredLog "SID validation failed or well-known SID detected: $sidString" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $null
        }

        Write-StructuredLog "SID $sidString passed validation, testing for orphaned status..." -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId

        # Test for orphaned status and create result
        $isOrphaned = Test-OrphanedSID -SID $sidString
        Write-StructuredLog "Orphaned status for SID $sidString : $isOrphaned" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId

        if ($isOrphaned) {
            Write-StructuredLog "ORPHANED SID DETECTED: $sidString on $ObjectDN" -Level Verbose -Component 'IdentityResolution' -CorrelationId $CorrelationId
            ...
            Write-StructuredLog "SID $sidString is valid (found in AD)" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $null
        }
    }
    catch {
        Write-StructuredLog "Error testing access rule for orphaned SID on $ObjectDN : $($_.Exception.Message)" -Level Warning -Component 'IdentityResolution' -CorrelationId $CorrelationId
        return $null
    }
}
...
function Resolve-IdentityReference {
    ...
    try {
        Write-StructuredLog "Resolving identity reference type for $ObjectDN" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
        ...
        if ($IdentityReference -is [System.Security.Principal.SecurityIdentifier]) {
            if ($IdentityReference.Value) {
                ...
                Write-StructuredLog "Extracted SID from SecurityIdentifier: $sidString" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
                return $sidString
            } else {
                Write-StructuredLog "SecurityIdentifier object has null Value property for $ObjectDN" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
                return $null
            }
        }
        ...
        if (Test-SIDFormat -SID $stringValue) {
            Write-StructuredLog "Extracted valid SID from identity reference: $stringValue" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $stringValue
        } else {
            Write-StructuredLog "Extracted string is not a valid SID format: $stringValue (Type: $($IdentityReference.GetType().Name)) for $ObjectDN" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $null
        }
    }
    catch {
        Write-StructuredLog "Error resolving identity reference for $ObjectDN : $($_.Exception.Message)" -Level Warning -Component 'IdentityResolution' -CorrelationId $CorrelationId
        return $null
    }
}
...
function Convert-NTAccountToSID {
    ...
    try {
        if (-not $NTAccount.Value) {
            Write-StructuredLog "NTAccount object has null Value property for $ObjectDN" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $null
        }

        Write-StructuredLog "Attempting to translate NTAccount: '$($NTAccount.Value)'" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
        ...
        if ($translatedSID -and $translatedSID.Value) {
            ...
            Write-StructuredLog "Successfully translated NTAccount '$($NTAccount.Value)' to SID: $sidString" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $sidString
        } else {
            Write-StructuredLog "Translation of NTAccount '$($NTAccount.Value)' returned null SID" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $null
        }
    }
    catch {
        # Translation failed - this indicates an orphaned account
        Write-StructuredLog "ORPHANED ACCOUNT DETECTED: Failed to translate NTAccount '$($NTAccount.Value)' to SID - likely orphaned: $($_.Exception.Message)" -Level Information -Component 'IdentityResolution' -CorrelationId $CorrelationId
...
function Test-SIDValidityAndType {
    ...
    try {
        ...
        if (-not (Test-SIDFormat -SID $SIDString)) {
            Write-StructuredLog "Invalid SID format: $SIDString" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $false
        }

        # Check for well-known SIDs (should be excluded from orphaned SID detection)
        if (Test-WellKnownSID -SID $SIDString) {
            Write-StructuredLog "Skipping well-known SID: $SIDString" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $false
        }

        Write-StructuredLog "SID $SIDString passed validation and is not well-known" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
        return $true
    }
    catch {
        Write-StructuredLog "Error validating SID $SIDString : $($_.Exception.Message)" -Level Warning -Component 'IdentityResolution' -CorrelationId $CorrelationId
        return $false
    }
}
...
function Get-StringFromIdentityReference {
    ...
    try {
        Write-StructuredLog "Extracting string from identity reference (Object: $ObjectDN)" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
        $stringValue = $null
        ...
        if ($IdentityReference -and $IdentityReference.PSObject.Properties['Value'] -and $IdentityReference.Value) {
            ...
            Write-StructuredLog "Extracted string from Value property: $stringValue" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
        }
        ...
        elseif ($IdentityReference -and $IdentityReference.ToString) {
            try {
                ...
                Write-StructuredLog "Extracted string from ToString method: $stringValue" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            } catch {
                Write-StructuredLog "Failed to convert identity reference to string using ToString for $ObjectDN : $($_.Exception.Message)" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            }
        }
        # Method 3: Direct string conversion (last resort)
            ...
            try {
                ...
                Write-StructuredLog "Extracted string using direct conversion: $stringValue" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            } catch {
                Write-StructuredLog "Unable to extract string value from identity reference for $ObjectDN : $($_.Exception.Message)" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
                return $null
            }
        }
        ...
        if ([string]::IsNullOrWhiteSpace($stringValue)) {
            Write-StructuredLog "Empty or null string value extracted from identity reference for $ObjectDN" -Level Debug -Component 'IdentityResolution' -CorrelationId $CorrelationId
            return $null
        }
        ...
        Write-StructuredLog "Error extracting string from identity reference for $ObjectDN : $($_.Exception.Message)" -Level Warning -Component 'IdentityResolution' -CorrelationId $CorrelationId
        return $null
    }
}
...
Write-StructuredLog "Resolve-SIDIdentity module loaded successfully" -Level Debug -Component 'IdentityResolution' -CorrelationId $([System.Guid]::NewGuid().ToString())
