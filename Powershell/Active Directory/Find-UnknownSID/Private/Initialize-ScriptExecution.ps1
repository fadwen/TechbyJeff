...
function Initialize-ScriptExecution {
    ...
    process {
        try {
            ...
            if ($LogPath) {
                Initialize-ScriptLogging -LogPath $LogPath -CorrelationId $CorrelationId -LogLevel $LogLevel -SuppressConsoleOutput:$SuppressConsoleOutput
            } else {
                $defaultLogPath = ".\Logs\Find-UnknownSID_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
                Initialize-ScriptLogging -LogPath $defaultLogPath -CorrelationId $CorrelationId -LogLevel $LogLevel -SuppressConsoleOutput:$SuppressConsoleOutput
            }
