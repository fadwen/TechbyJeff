...
function Invoke-MainProcessingLogic {
    ...
    begin {
        ...
        Write-StructuredLog "Initialized streaming results manager with temp directory: $tempDir" -Level Debug -Component 'MainProcessing' -CorrelationId $CorrelationId
    }
                ...
                $results = $objectArray | ForEach-Object {
                    try {
                        ...
                        if ($processedCount % 100 -eq 0) {
                            # Use centralized aggressive cleanup for large operations
                            $cleanupResult = Invoke-AggressiveCleanup -CorrelationId $CorrelationId
