...
function Get-SafeFileName {
    ...
    .NOTES
        ...
        - Illegal characters (\/:*?"<>|) → underscore (_)
        - Multiple whitespace → single underscore (_)
        - Leading/trailing spaces → trimmed
        - Empty/null strings → "Unknown"
        - Length exceeded → truncated with "_truncated" suffix
    ...
    process {
        try {
            ...
            if ([string]::IsNullOrWhiteSpace($InputString)) {
                Write-StructuredLog "Safe filename generation: Input string was empty, using default" -Level Debug -Component 'String-Utilities' -CorrelationId $CorrelationId
                return "Unknown"
            }
            ...
            if ($safeName.Length -gt $MaxLength) {
                ...
                if ($maxContentLength -gt 0) {
                    ...
                    Write-StructuredLog "Safe filename generation: Truncated filename from $($InputString.Length) to $MaxLength characters" -Level Debug -Component 'String-Utilities' -CorrelationId $CorrelationId
                } else {
            ...
            if ([string]::IsNullOrWhiteSpace($safeName) -or $safeName -eq '_') {
                ...
                Write-StructuredLog "Safe filename generation: Generated fallback filename due to invalid result" -Level Warning -Component 'String-Utilities' -CorrelationId $CorrelationId
            }

            Write-StructuredLog "Safe filename generation successful: '$InputString' → '$safeName'" -Level Debug -Component 'String-Utilities' -CorrelationId $CorrelationId
            return $safeName
        }
        catch {
            Write-StructuredLog "Error creating safe filename from '$InputString': $($_.Exception.Message)" -Level Warning -Component 'String-Utilities' -CorrelationId $CorrelationId
...
Write-StructuredLog "String-Utilities module loaded successfully" -Level Debug -Component 'String-Utilities' -CorrelationId ([System.Guid]::NewGuid().ToString())
