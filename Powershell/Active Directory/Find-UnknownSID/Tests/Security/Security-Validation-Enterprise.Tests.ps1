...
[PSCustomObject]@{
Pattern = #Requires -Module Pester
...
Mock Remove-Item { 
param($Path, [switch]$Recurse, [switch]$Force)
$systemPaths = @('^C:\\Windows', '^C:\\Program Files', '^\\\\', '^C:\\
...
, '^/etc', '^/usr', '^/var')
if ($systemPaths | Where-Object { $Path -match #Requires -Module Pester
...
param($Uri, $Method = 'GET', $Body, $Headers)
$suspiciousPatterns = @('evil\.com', 'malware\.', 'phishing\.', 'attack\.', '\.onion')
if ($suspiciousPatterns | Where-Object { $Uri -match #Requires -Module Pester
