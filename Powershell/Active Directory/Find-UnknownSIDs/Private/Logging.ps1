...
<#
.SYNOPSIS
    Enterprise logging system for Find-UnknownSIDs operations
...
function Initialize-ScriptLogging {
    <#
    .SYNOPSIS
        Initializes the enterprise logging system for Find-UnknownSIDs
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($LogPath)) {
            ...
            $LogPath = ".\Logs\Find-UnknownSIDs_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
        }
...
"@
        ...
        $script:LogPath = Join-Path (Get-Location) "Find-UnknownSIDs_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
        $script:LogFileInitialized = $false
    }
}
# Add this new function to c:\temp\Find-UnknownSIDs\Private\Logging.ps1
...
function Write-SecurityLog {
    ...
    Write-ScriptLog "Security event: $SecurityEventType - $Message (Outcome: $Outcome)" -Level Debug -Component 'Security' -CorrelationId $CorrelationId
...
function Write-ScriptLog {
    <#
    .SYNOPSIS
        Wrapper function for Write-StructuredLog to maintain compatibility

    .DESCRIPTION
        Provides a simplified interface to Write-StructuredLog for script-level
        logging operations. Maintains backward compatibility while leveraging
        the full enterprise logging capabilities including automatic message sanitization.

    .PARAMETER Message
        The log message content (will be sanitized for security)

    .PARAMETER Level
        Logging level (Information, Warning, Error, etc.)
    ...
    .PARAMETER CorrelationId
        Correlation ID for tracking (uses script-level if not provided)

    .PARAMETER Data
        Additional structured data

    .EXAMPLE
        PS> Write-ScriptLog -Message "Operation completed" -Level Information -Component "Main"

        Writes an informational log entry using structured logging with sanitized message

    .NOTES
        This is a wrapper around Write-StructuredLog for compatibility.
        Uses the script-level correlation ID automatically.
        Messages are automatically sanitized to prevent log injection attacks.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    # Message sanitization is handled by Write-StructuredLog
    # Use provided CorrelationId or fall back to script-level
    if (-not $CorrelationId -and $script:CorrelationId) {
        $CorrelationId = $script:CorrelationId
    }

    # Apply the same log level filtering as Write-StructuredLog
    $logLevelOrder = @{
        'Critical' = 0
        'Error' = 1
        'Warning' = 2
    ...
    $currentLogLevel = if ($script:LogLevel) { $script:LogLevel } else { 'Information' }

    # Skip logging if the message level is lower priority than the current log level
    if ($logLevelOrder[$Level] -gt $logLevelOrder[$currentLogLevel]) {
        return
    }

    # Call the main structured logging function directly with individual parameters
    # Sanitization will be handled by Write-StructuredLog
    if ($CorrelationId) {
        Write-StructuredLog -Message $Message -Level $Level -Component $Component -Color $Color -Data $Data -CorrelationId $CorrelationId
    } else {
        Write-StructuredLog -Message $Message -Level $Level -Component $Component -Color $Color -Data $Data
    }
}
