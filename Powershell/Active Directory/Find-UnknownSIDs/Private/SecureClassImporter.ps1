...
function Import-ProjectClassesSecure {
    ...
    begin {
        ...
        # Generated on 2025-07-02 from Find-UnknownSIDs project class files
        $approvedClasses = @{
        ...
        if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
            Write-ScriptLog "Starting secure class import with integrity validation: $ValidateIntegrity" -Level Information -Component 'SecureClassImporter' -CorrelationId $CorrelationId
        }
    }
    ...
    process {
        try {
            ...
            foreach ($className in $approvedClasses.Keys) {
                ...
                if ($PSCmdlet.ShouldProcess($className, "Import Class File")) {
                    try {
                        ...
                        if ($ValidateIntegrity) {
                            ...
                            if ($actualHash.Hash -ne $classInfo.ExpectedHash) {
                                ...
                                # Log security event
                                if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
                                    Write-ScriptLog $hashMismatchMessage -Level Warning -Component 'SecureClassImporter' -CorrelationId $CorrelationId
                                }
                                ...
                                if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
                                    Write-ScriptLog "File integrity verified for: $className" -Level Debug -Component 'SecureClassImporter' -CorrelationId $CorrelationId
                                }
                            }
                        }
                        ...
                        # 4. Load the class file - use ExecutionContext to load in caller's scope
                        Write-Verbose "Loading class file: $className"
                        $classContent = Get-Content -Path $resolvedClassPath.Path -Raw
                        $ExecutionContext.InvokeCommand.InvokeScript($classContent)
                        ...
                        if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
                            Write-ScriptLog "Successfully imported class: $className" -Level Debug -Component 'SecureClassImporter' -CorrelationId $CorrelationId
                        }
                        ...
                        if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
                            Write-ScriptLog $errorMessage -Level Error -Component 'SecureClassImporter' -CorrelationId $CorrelationId
                        }
            ...
            if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
                Write-ScriptLog "Secure class import process failed: $($_.Exception.Message)" -Level Error -Component 'SecureClassImporter' -CorrelationId $CorrelationId
            }
            throw
        }
    }
        ...
        if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
            Write-ScriptLog "Secure class import completed: $successCount/$totalCount classes loaded successfully" -Level Information -Component 'SecureClassImporter' -CorrelationId $CorrelationId

            if ($failureCount -gt 0) {
                $failureDetails = $failedClasses | ForEach-Object { "$($_.ClassName): $($_.Error)" }
                Write-ScriptLog "Failed classes: $($failureDetails -join '; ')" -Level Error -Component 'SecureClassImporter' -CorrelationId $CorrelationId
            }

            if ($securityViolationCount -gt 0) {
                Write-ScriptLog "Security violations detected: $($securityViolations -join '; ')" -Level Warning -Component 'SecureClassImporter' -CorrelationId $CorrelationId
            }

            if ($ValidateIntegrity) {
                $integrityFailures = ($integrityResults | Where-Object { -not $_.IntegrityValid }).Count
                Write-ScriptLog "Integrity verification results: $($integrityResults.Count - $integrityFailures)/$($integrityResults.Count) files passed" -Level Information -Component 'SecureClassImporter' -CorrelationId $CorrelationId
            }
        }
    ...
    process {
        foreach ($className in $classTests.Keys) {
            try {
                ...
                # Log successful test
                if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
                    Write-ScriptLog "$className instantiation test passed" -Level Debug -Component 'ClassTester' -CorrelationId $CorrelationId
                }
            }
            catch {
                $testResults += [PSCustomObject]@{
                ...
                # Log test failure
                if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
                    Write-ScriptLog "$className instantiation test failed: $($_.Exception.Message)" -Level Error -Component 'ClassTester' -CorrelationId $CorrelationId
                }
            }
        }
    }
        ...
        # Summary logging
        if (Get-Command Write-ScriptLog -ErrorAction SilentlyContinue) {
            Write-ScriptLog "Class instantiation testing completed: $successCount/$totalCount classes passed" -Level Information -Component 'ClassTester' -CorrelationId $CorrelationId
            ...
            if ($IncludePerformanceTesting -and $performanceData.Count -gt 0) {
                ...
                Write-ScriptLog "Average instantiation time: $([math]::Round($avgInstantiationTime, 2))ms" -Level Information -Component 'ClassTester' -CorrelationId $CorrelationId
            }
        }
